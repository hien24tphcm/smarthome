...
async function addMember() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            console.warn("POST /users/ không OK:", res.status, text);

            // Workaround: BE có thể đã insert user nhưng lỗi response_model nên trả 500
            const verifyUser = await verifyCreatedMember(email);
            ...
            if (verifyUser) {
                ...
                if (data && data.detail) {
                    ...
                    msg = "Backend lỗi 500 khi tạo thành viên. User chưa được tạo hoặc lỗi response_model.";
        ...
        // Workaround cho trường hợp browser báo Failed to fetch do BE trả 500 thiếu CORS header
        const verifyUser = await verifyCreatedMember(email);
        ...
        if (verifyUser) {
            ...
            showToast("Không gọi được API tạo thành viên. Kiểm tra backend logs.", "error");
            return;
        }
    }
    ...
    showToast(
        `Đã thêm thành viên ${(createdUser?.fname || fname)} ${(createdUser?.lname || lname)}`,
        "success"
    );
...
async function verifyCreatedMember(email) {
    try {
        ...
        console.error("verifyCreatedMember error:", err);
        return null;
    }
}

function getMemberId(u) {
    return (
        u.id ||
        u.uid ||
        u.user_id ||
...
async function loadMembers() {
    ...
    try {
        const res = await fetch(`${API}/users/home/members`, {
            method: "GET",
            headers: authHeaders()
        });
        ...
        if (!res.ok) {
            const errText = await res.text();
            console.error("loadMembers error:", res.status, errText);
        ...
        const adminCount = users.filter(u => {
            const userType = (u.type || u.user_type || "").toLowerCase();
        ...
        tbody.innerHTML = users.map(u => {
            const memberId = getMemberId(u);
            const fullName = `${u.fname || ""} ${u.lname || ""}`.trim();
            const userType = (u.type || u.user_type || "").toLowerCase();
        
            const roleBadge = userType === "admin"
            ...
            const statusValue = u.status === true || u.status === "active" || u.status === 1;
            ...
            const isLastAdmin = userType === "admin" && adminCount <= 1;
        
            let deleteButton = "";
        
            if (isCurrentUser) {
                deleteButton = `<span class="status-warning">Tài khoản hiện tại</span>`;
            } else if (isLastAdmin) {
                deleteButton = `<span class="status-warning">Admin cuối cùng</span>`;
            } else {
                deleteButton = `
                    <button class="btn-trash"
                            onclick="deleteMember('${memberId}', '${escapeHtml(fullName)}')"
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="6" class="empty-state">
                    <i class="fas fa-users"></i>
                    Lỗi kết nối khi tải thành viên
                </td>
            </tr>
        `;
    }
...
async function deleteMember(userId, userName = "") {
    if (!userId || userId === "undefined" || userId === "null") {
        showToast("Không xác định được ID thành viên", "error");
        console.error("Không có userId để xoá:", userId);
        return;
    }

    const ok = confirm(
        `Bạn có chắc muốn xoá thành viên "${userName}"?\nHành động này không thể hoàn tác.`
    );
    ...
    try {
        ...
        if (!res.ok) {
            const errText = await res.text();
            console.warn("deleteMember failed:", res.status, errText);

            let msg = "Không thể xoá thành viên";
            ...
            try {
                ...
                if (String(msg).includes("Cannot delete last admin")) {
                    msg = "Không thể xoá admin cuối cùng trong nhà.";
                }
            } catch (_) {}
        }

        showToast(`Đã xoá thành viên "${userName}"`, "success");
        await loadMembers();

    } catch (err) {
        console.error("deleteMember error:", err);
        showToast("Lỗi kết nối khi xoá thành viên", "error");
    }
}
